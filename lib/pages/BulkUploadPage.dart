...
import '../database_Module/database_Manager.dart';
...
class _BulkUploadPageState extends State<BulkUploadPage> {
  ...
  String? error;
  String? _backupPathText;
  final databaseManager = ObjectBoxManager();
  ...
  Future<void> _setBackupPathText() async {
    final directory = await getDownloadsDirectory();
    setState(() => _backupPathText = directory?.path);
  }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ElevatedButton.icon(
              onPressed: () => databaseManager.handleBackup(context),
              // icon: Icon(Icons.upload_file),
              label: Text("1. Back Up Database"),
            ),
            if (_backupPathText != null)
              Text(
                  p.join("File Created on Below Path:-"),
                  softWrap: true,
                  style: TextStyle(color: const Color.fromARGB(255, 255, 0, 0), fontSize: 12),
                ),
              Padding(
                padding: const EdgeInsets.only(top: 0.0, left: 4.0,bottom: 4.0),
                child: Text(
                  p.join(_backupPathText ?? "", 'backups', 'backup_${DateFormat('dd_MM_yyyy').format(DateTime.now())}.mdb'),
                  softWrap: true,
            ...
            ElevatedButton.icon(
              ...
              // icon: Icon(Icons.upload_file),
              label: Text("3. Upload CSV File"),
            ),
            ElevatedButton(
              onPressed: _exportSampleCSV,
              child: Text("4. Download Sample CSV"),
            ),
            if (error != null)
