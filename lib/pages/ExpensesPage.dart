  ...
  void _printAllExpensesFromBox() async {
    ...
    if (allExpenses.isEmpty) {
      ...
      for (var i = 0; i < allExpenses.length; i++) {
        final expenseEntity = allExpenses[i];
        final expenseMap = jsonDecode(expenseEntity.expence);
        final jsonId = expenseMap['id'];
        print_log(
          'Expense #${i + 1} (ObjectBox ID: ${expenseEntity.id}) | JSON ID: $jsonId | Data: ${expenseEntity.expence}',
        );
      }
    }
    print_log('--- End of expenses list ---');
  }
...
void _addNewExpense() {
  ...
  void _saveExpense() {
    ...
    // final mappp = newExpense.toMap();

    setState(() => _expenses.insert(0, newExpense));
    _saveExpensesToPrefs(newExpense:newExpense);
    _clearForm();
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
