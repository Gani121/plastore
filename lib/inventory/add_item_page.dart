...
class _AddItemPageState extends State<AddItemPage> {
  ...
  final TextEditingController barCode2Controller = TextEditingController();
    ...
    // Controllers for product Display
  ...
  void initState() {
    ...
    if (widget.store != null) {
      ...
      print_log("⚠️ Store is null! _menuItemBox not initialized.");
    }
    ...
    if (widget.item != null) {
      ...
      _imagePath = "/storage/emulated/0/Android/data/${AppConstants.test_version}/files/pictures/menu_images/${item.name}.jpeg";
      File imageFile = File(_imagePath ?? "");
      bool hasImage = imageFile.existsSync();
          // Check if the file exists
      if (!hasImage) {
        _imagePath = "/storage/emulated/0/Android/data/${AppConstants.test_version}/files/pictures/menu_images/${item.name}.jpg";
        File imageFile = File(_imagePath ?? "");
     ...
     // _imagePath = item.imagePath;
      ...
      // For a new item, find the highest existing ID and add 1.
      // This is more reliable than using the total item count.
      final query = _menuItemBox.query().order(MenuItem_.id, flags: Order.descending).build();
      final lastItem = query.findFirst();
      query.close();
      final nextId = (lastItem?.id ?? 0) + 1;

      itemCodeController.text = nextId.toString();
    }
  }
  ...
  void dispose() {
    ...
    // Parking fields
    acSellPrice1Controller.dispose();
    acSellPrice1ControllerHalf.dispose();
    nonAcSellPrice1Controller.dispose();
    nonAcSellPrice1ControllerHalf.dispose();
    onlineDeliveryPriceController.dispose();
    onlineDeliveryPriceControllerHalf.dispose();
    onlineSellPriceController.dispose();
    onlineSellPriceControllerHalf.dispose();
    ...
    barCode2Controller.dispose();
    availableController.dispose();
    adjustStockController.dispose();
