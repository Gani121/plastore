...
class _DetailPageState extends State<DetailPage> {
  ...
  late final TextEditingController _mobileNoController = TextEditingController();
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.mode == "edit") {
        ...
        if (widget.transaction != null && (widget.transaction ?? {}).isNotEmpty) {
          ...
          final _ordertype = transaction['orderType'] ?? 'Dine-In';
  ...
  void addtablecart(CartProvider cartProvider) async {
    ...
    // 2. Prepare data
    final tableNo = widget.table!['kot'] as int;
...
class __BottomBarState extends State<_BottomBar> {
  ...
  void didUpdateWidget(covariant _BottomBar oldWidget) {
    ...
    // If the parent passes a different ordertype (e.g. changes from 'Dine-In' to 'Zomato')
    // we must update the local state variable.
    if (widget.ordertype != oldWidget.ordertype ) {
      setState(() {
        _orderType = widget.ordertype;
      });
      print_log("Bottom _orderType updated via parent to:- $_orderType");
    }
    if (widget.mobileNo != oldWidget.mobileNo || widget.name != oldWidget.name || widget.adreess != oldWidget.adreess) {
      setState(() {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ListTile(
                leading: Icon(Icons.qr_code_scanner_rounded, color: Colors.deepPurple.shade400, size: 30),
                title: Text('Other', style: TextStyle(fontSize: 16)),
  ...
  Future<Map<String, double>?> _showSplitPaymentDialog(BuildContext context, double totalAmount) async {
    ...
    return showDialog<Map<String, double>?>(
      ...
      builder: (context) {
        // Use a StatefulBuilder to update the UI within the dialog
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            cashController.addListener(_updateState);
            upiController.addListener(_updateState);

            final cashAmount = double.tryParse(cashController.text) ?? 0.0;
            final upiAmount = double.tryParse(upiController.text) ?? 0.0;
            final bool isAmountMismatch = totalAmount != (cashAmount + upiAmount);
            ...
            return AlertDialog(
              ...
              content: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 16),
                    // Show the warning text only if the amounts don't match
                    if (isAmountMismatch)
                      Text(
                        "Cash and UPI amounts do not match the total.",
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    cashController.removeListener(_updateState);
                    upiController.removeListener(_updateState);
                ...
                ElevatedButton(
                  onPressed: () {
                    // if (!isAmountMismatch) {
                      cashController.removeListener(_updateState);
                      upiController.removeListener(_updateState);
                      Navigator.of(context).pop({'cash': cashAmount, 'upi': upiAmount});
                    // } else if (isAmountMismatch) {
                    //   screen_massage(context, "Amounts do not match total.");
                    // }
                  },
                  child: Text('Confirm'),
                ),
                ...
                builder: (context, serviceCharge, _) {
                  ...
                  return Column(
                    ...
                    children: [
                            ...
                            children: () {
                              return deliveryTypes.map((type) {
                                return Padding(
                                  ...
                                  child: SizedBox(
                                    height: 30, // Constrain the height of the chip
                                    child: ChoiceChip(
                                      label: Text(type),
                                      labelStyle: TextStyle(fontSize: 12),
                                      padding: EdgeInsets.symmetric(horizontal: padding_chip),
                                      showCheckmark: false,
                      ...
                      Row( // Row for Total and Received
                        ...
                        children: [
                          ...
                          Checkbox(
                            ...
                            onChanged: (bool? newValue) {
                              setState(() {
                                _isChecked = newValue!;
                                print_log("_isChecked ${!_isChecked}");
                              });
                            },
                          ),
                          if (_isChecked)
                                  ...
                                  try {
                                    if(tableno > 0){
                                      ...
                                      await printer.printCart(context: context,
                                                      ...
                                                      total:total.toInt(),
                                                      mode:"print",
                                                      payment_mode: (ttid != null) ? 'print_${transactiondata['id']}' : 'print',
                                                      transactionData : transactiondata,
                                                      tableNo: tableno,
                                                      );
                                      } else {
                                        ...
                                        await printer.printCart(
                                          ...
                                          mode: "print",
                                          transactionData : transactiondata,
                                          payment_mode: (widget.mode == 'edit') ? 'print_${transactiondata['id']}' : 'print',
                                        );
                                      }
                            ...
                            Expanded(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: _isPrinting ? null : () async {
                                  ...
                                  try {
                                    String? paymentMode;
                                    final existingPaymentMode = transactiondata['payment_mode'] as String?;

                                    if (existingPaymentMode != null && ['cash', 'upi', 'other'].contains(existingPaymentMode.toLowerCase())) {
                                      paymentMode = existingPaymentMode;
                                    } else {
                                      paymentMode = await _showPaymentMethodDialog(context);
                                      if (paymentMode == 'other' && mounted) {
                                        final splitAmounts = await _showSplitPaymentDialog(context, total.toDouble());

                                        if (splitAmounts != null) {
                                          print_log("splitAmounts $splitAmounts $paymentMode");
                                          transactiondata['cashamount'] = splitAmounts['cash'];
                                          transactiondata['upiamount'] = splitAmounts['upi'];
                                        } else {
