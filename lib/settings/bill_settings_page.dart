...
class _billSettingsPageState extends State<billSettingsPage> {
  ...
  void initState() {
    ...
    _loadSelectedStyle();
    get_billing_type();
    // 2. Load the custom list when app starts
    loadOrderTypes();
  ...
  // --- SAVE/LOAD LOGIC FOR ORDER TYPES ---
  Future<void> saveOrderTypes() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList('order_type_list', order_type);
  }
  ...
  Widget _buildDynamicDropdownTile(String title) {
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: DropdownButton<String>(
            ...
            items: [
              // 1. Map existing items to DropdownMenuItems
              ...order_type.map((String value) {
              ...
              // 2. Add the "Add New" option at the bottom
              const DropdownMenuItem<String>(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text("Add New Order Type...", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ],
  ...
  void _deleteOrderType(String itemToDelete) {
    setState(() {
      order_type.remove(itemToDelete);
      // If the deleted item was selected, reset selection
      if (selectedOrderType == itemToDelete) {
        selectedOrderType = order_type.isNotEmpty ? order_type.first : null;
      }
    });
    saveOrderTypes(); // Update Prefs
    // Hack to close the dropdown programmatically if needed, usually setState rebuilds it
  ...
  void _showAddDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text("Add New Order Type"),
          content: TextField(
            controller: addController,
            decoration: const InputDecoration(hintText: "Enter name (e.g. UberEats)"),
          ),
          actions: [
            TextButton(
            ...
            ElevatedButton(
              onPressed: () {
                if (addController.text.isNotEmpty) {
                  setState(() {
                    order_type.add(addController.text);
                    selectedOrderType = addController.text; // Select the new item
                  });
                  saveOrderTypes(); // Update Prefs
